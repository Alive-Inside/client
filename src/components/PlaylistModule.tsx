...
export type CurrentlyPlaying = {
  ...
  trackId: string;
  state: "playing" | "not playing" | "frame" | "initial buffer" | "buffering";
};
const fakeTrack: LoadableTrack = {
  ...
  artist: { id: "", name: "Fake artist", url: "" },
  id: ``,
  mp3PreviewUrl: "",
  title: "Fake title",
  uri: "",
...
export default function PlaylistModule({
  ...
  spotifyUserData,
  searchType,
  generatedPlaylistTracks: generatedTracks,
  isFinalPlaylist,
  addTrack,
  addArtist,
  removeTrack,
  ...
  addArtist?: Function;
  children?: any;
  isFinalPlaylist?: boolean;
  generatedPlaylistTracks?: Track[];
  spotifyUserData: SpotifyUserData;
  searchType: { item: "track" | "artist"; multiple: boolean };
  isQuickList?: boolean;
  startAgain?: Function;
}) {
  const [tracks, setTracks] = useState<(Track & { loading?: boolean })[]>(
    generatedTracks ?? [
      fakeTrack,
      fakeTrack,
      fakeTrack,
  ...
  const [activateFocus, { toggle: toggleFocus }] = useDisclosure(false);
  ...
  useEffect(() => {
    ...
    setCurrentlyPlaying({
      ringProgress: 0,
      mp3PreviewUrl: currentlyPlaying?.mp3PreviewUrl,
      state: "not playing",
  ...
  const onAddFive = useCallback(
    async (trackId: string) => {
      try {
        ...
        const recommendedTracks = await GetRecommendations(5, spotifyUserData.countryCode, {
          targetYear: releaseYear,
          trackIDs: [trackId],
          onlyIncludeFromArtistID: tracks.find((t) => t.id === trackId).artist.id,
          duplicateTrackIDsToAvoid: tracks.map((lT) => lT.id),
        });
        ...
        if (spotifyPlaylist !== null)
          await AddTracksToPlaylist(
            spotifyPlaylist.id,
            recommendedTracks.map((t) => t.uri),
            spotifyUserData.accessToken,
            insertIndex
          );
  ...
  async function exportPlaylist() {
    let customPlaylistName: string;
    if (isQuickList) {
      customPlaylistName = window.prompt("Playlist name?");
    }
    setIsSavingToSpotify(true);
    const { url, id } = await createPlaylist({
      eldersFirstName: formValues?.eldersFirstName,
      eldersBirthYear: formValues?.eldersBirthYear,
      musicalPreferences: formValues?.musicalPreferences,
      customPlaylistName,
    });
  ...
  const onTogglePlaying = useCallback(
    (trackId: string, mp3PreviewUrl: string) => {
      if (currentlyPlaying?.trackId === trackId) {
        ...
        setCurrentlyPlaying({
          ...
          trackId,
          mp3PreviewUrl,
          state: currentlyPlaying?.state !== "not playing" ? "not playing" : "initial buffer",
        });
      } else {
    ...
    <div style={{ padding: "1em" }}>
      {currentlyPlaying && (
        <ReactPlayer
          ...
          style={{ display: "none" }}
          onEnded={onEnd}
          url={currentlyPlaying?.mp3PreviewUrl}
          playing={currentlyPlaying?.state !== "not playing"}
        />
      )}
      <div>
        <Container
        ...
        >
          ...
          <FocusTrap active={activateFocus}>
            <TextInput
              autoFocus={true}
              size="md"
              style={{ margin: `${!isFinalPlaylist ? "0 auto" : "0 0 0 1rem"}` }}
              value={searchQuery}
              className="searchBar"
              autoComplete="off"
              onChange={(e) => {
              ...
              icon={
                isLoadingSearchQuery ? (
                  <Loader size={17} color="black" style={{ marginBottom: "3px" }} />
                ) : (
                  <ActionIcon
                    variant="transparent"
                    style={{ marginLeft: "0.25rem", marginBottom: "0" }}
                  >
                    <IconSearch color="black" className="searchIcon" />
                  </ActionIcon>
                )
              }
              placeholder={`Search for ${searchType.item === "artist" ? "an artist" : "a song"}`}
              rightSection={
                searchQuery && (
                  <div
          ...
          {isFinalPlaylist && (
            <div style={{ display: "flex", flexDirection: "row", margin: "1rem" }}>
              {!isSavingToSpotify ? (
                  ...
                  <a target="_blank" rel="noreferrer" href={spotifyPlaylist?.url}>
                    <Skeleton visible={loadingSpotifyPlaylist} radius={"xl"}>
                      {tracks.length > 0 && (
                        <Button
                          color="green"
                          onClick={() => !spotifyPlaylist && exportPlaylist()}
                          radius={"xl"}
                          variant="outline"
                        ...
                        >
                          {spotifyPlaylist ? (
                            <>
                              <Image
                                style={{ height: "21px", width: "21px", marginRight: "0.5rem" }}
                                src={SpotifyIcon.src}
                              />{" "}
                              Open Playlist{" "}
                            </>
                ...
                <Loader
                  style={{ marginLeft: "0.5em", marginRight: "1rem", marginTop: "0.75rem" }}
                  variant="dots"
                  color="green"
                />
              )}
              {spotifyPlaylist !== null && !isSavingToSpotify && !isQuickList && (
                <Link href="/memory-bank">
                  <Button color="blue" radius={"xl"} variant="outline" style={{}}>
                    <IconDeviceFloppy style={{ marginRight: "0.5rem" }} />
                    Save to Memory Bank
                  </Button>{" "}
                </Link>
              )}
        ...
        <Center style={{ width: "100%" }}>
          {(noResultsFound ||
            ...
            tracks.filter((x) => !x.fake).length > 0 ||
            artists.filter((x) => !x.fake).length > 0) && (
            <Paper
              shadow="xs"
              style={{
                paddingLeft: "5px",
            ...
            >
              <ScrollArea.Autosize maxHeight={"45vh"} viewportRef={viewport}>
                <div>
                  {noResultsFound &&
                    !isLoadingSearchQuery &&
                    [...trackSearchResults, ...artistSearchResults].length === 0 && (
                      <SpotifyRow onTogglePlaying={onTogglePlaying}>
                        <Text style={{ fontWeight: "bold", wordWrap: "break-word", width: "100%" }}>
                  ...
                  {searchType.item === "track" &&
                    ...
                    (searchQuery.length === 0 || trackSearchResults.length === 0) &&
                    !isLoadingSearchQuery &&
                    tracks.map((track, i) => {
                      return (
                        <AddedTrackRow
                          ...
                          key={i}
                          fiveTracksLoading={fiveTracksLoading.get(track.id)}
                          onAddFive={onAddFive}
                          isFinalPlaylist={isFinalPlaylist}
                          track={track}
                  ...
                  {searchType.item === "artist" &&
                    (searchQuery.length === 0 || artistSearchResults.length === 0) &&
                    !isLoadingSearchQuery &&
                  ...
                  <ArtistResults
                    ...
                    existingArtistIDs={artists.map((a) => a.id)}
    ...
    try {
      if (query.length > 0) {
        ...
        const results: any = await Search(
          ...
          spotifyUserData.accessToken,
          { type: searchType.item === "artist" ? "artists" : "tracks", limit: searchLimit }
        );
