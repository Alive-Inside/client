...
export default function AppShell({ children }: { children: any }) {
  ...
  const mainLinks: LinkProps[] = [
    ...
    { label: "Donate", link: "https://aliveinside.org/posts/124120/donate" },
    { label: "Aliveinside.org", link: "http://aliveinside.org" },
  ];
    ...
    async function LoginCheck() {
      ...
      if (router.query.logout) {
        ...
        if (router.query.jwt) {
          ...
          setUser({
            avatar: spotifyUserData.avatar?.url ?? NO_SPOTIFY_AVATAR_IMAGE,
            name: spotifyUserData.name,
          });
          router.replace("/app");
        } else {
          try {
            const spotifyUserDataItem = getSpotifyUserData();
  ...
  return (
    <Header height={HEADER_HEIGHT} mb={120}>
      <Link href="/">
        <Image
          className={classes.aliveInsideLogo}
          style={{ position: "absolute", left: "20px", marginTop: "9px" }}
          src={AliveInsideLogo}
          alt="Alive Inside Logo"
        />
      </Link>
      <Flex>
...
export function getSpotifyUserData() {
  return localStorage.getItem("spotifyUserData");
}
